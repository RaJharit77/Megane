...
const Flower3D = dynamic(() => import('@/components/Flower3D'), { ssr: false });
...
export default function Home() {
  ...
  const handleReset = () => setFlower(null);
      ...
      <div className="max-w-6xl mx-auto bg-white/95 rounded-3xl shadow-2xl shadow-pink-200/50 overflow-hidden">
        ...
        <main className="p-4 md:p-8">
          <AnimatePresence mode="wait">
            {!flower ? (
              <FormSection onGenerate={handleGenerateFlower} isGenerating={isGenerating} />
            ) : (
              <ResultSection flower={flower} onReset={handleReset} />
            )}
          </AnimatePresence>
        </main>
        ...
        <footer className="py-8 px-4 border-t-2 border-dashed border-pink-200 text-center">
          ...
          <p className="max-w-2xl mx-auto text-gray-600 mb-6">
            Cette application génère des fleurs réalistes uniques avec des messages personnalisés. Parfait pour exprimer
            vos sentiments de manière créative et magique !
          </p>
          <div className="flex flex-wrap justify-center gap-6 md:gap-10">
            {['🌸 Fleurs réalistes', '💝 Messages personnalisés', '📥 Téléchargement HD gratuit', '📄 Export PDF'].map(
              (feature, idx) => (
